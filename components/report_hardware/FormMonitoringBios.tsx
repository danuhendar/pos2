...
import { ReportMonitoringBios } from "@/controller/report_hardware/MonitoringReportHardware";
import IconSend from "../Icon/IconSend";
import themeConfig from "@/theme.config";
import AntiScrapedShieldComponent from "../shield/AntiScrapedShieldComponent";

interface FormMonitoringBiosProps {
    url: string,
    command: string,
    IDReport: string,
}
const FormMonitoringBios: React.FC<FormMonitoringBiosProps> = ({ url, command, IDReport }) => {
    ...
    const [active_reload, setActiveReload] = useState(true);
    ...
    const Def_Columns_ReportMonitoringBios = (rows:any) => {
            ...
            {
                field: 'KETERANGAN', headerName: 'KETERANGAN', width: 180, minWidth: 180, maxWidth: 180,
                renderCell: (cellValues: any) => {
                    return (
            ...
            { field: 'KDCAB', headerName: 'KDCAB', flex: 0, width: 60, minWidth: 60, maxWidth: 60 },
            { field: 'KDTK', headerName: 'KDTK', flex: 0, width: 80, minWidth: 80, maxWidth: 80 },
            { field: 'NAMA', headerName: 'NAMA', flex: 0, width: 250, minWidth: 250, maxWidth: 250 },
            { field: 'STATION', headerName: 'STATION', flex: 0, width: 90, minWidth: 90, maxWidth: 90 },
            { field: 'IP', headerName: 'IP', flex: 0, width: 150, minWidth: 150, maxWidth: 150 },
            { field: 'MANUFACTURER', headerName: 'MANUFACTURER', flex: 0, width: 250, minWidth: 250, maxWidth: 250,headerAlign:'center',align:'center'},
            { field: 'NAME', headerName: 'NAME', flex: 0, width: 250, minWidth: 250, maxWidth: 250,headerAlign:'center',align:'center' },
            { field: 'SERIAL_NUMBER', headerName: 'SERIAL_NUMBER', flex: 0, width: 200, minWidth: 200, maxWidth: 200,headerAlign:'center',align:'center' },
            { field: 'VERSION', headerName: 'VERSION', flex: 0, width: 200, minWidth: 200, maxWidth: 200,headerAlign:'center',align:'center' },
        ];
            ...
            SendHandleRowClick(IDReport,kdcab,kdtk,station,res_command,Token).then((response) => {
                ...
                if(parseFloat(code) === 200){
                    ...
                    if(res_data_code === 200){
                        const res_data_inti = parse_data_inti[0].data.split('"').join('').split(';');
                        ...
                        try {
                            data_prev[objIndex].MANUFACTURER = res_data_inti[0];
                            data_prev[objIndex].NAME = res_data_inti[1];
                            data_prev[objIndex].SERIAL_NUMBER = res_data_inti[2];
                            data_prev[objIndex].VERSION = res_data_inti[3];
                        } catch (Ex) {
                            data_prev[objIndex].MANUFACTURER = '-';
                            data_prev[objIndex].NAME = '-';
                            data_prev[objIndex].SERIAL_NUMBER = '-';
                            data_prev[objIndex].VERSION = '-';
                        }
                        var rows = data_prev;
                    }else{
                    ...
                    if (code === 200 || code === 209) {
                            ...
                            try {
                                ...
                                for (var o = 0; o < res_new.length; o++) {
                                    ...
                                    //-- MONITORING BIOS --//
                                    const obj = ReportMonitoringBios(false,0,[],res_kdcab,res_kdtk,res_nama,res_ip,res_station,res_request,res_response,IDReport,res_data_code,res_data_msg,result)
                                    rows.push(obj[0])
                                    if (res_data_code !== 200) {
                                ...
                                let arr_res_columns = Def_Columns_ReportMonitoringBios(rows)
                            ...
                            if (((msg.includes('Mohon di tunggu'))) || (msg.includes('Handletime kami Default'))) {
                                ...
                                try {
                                    ...
                                    for (var o = 0; o < res_data.length; o++) {
                                        ...
                                        const res_station = parse_data_inti.station;
                                        const res_ip = parse_data_inti.ip;
                                        const result = parse_data_inti.data.split('"').join('').split(';');
                                        const objIndex = data_rows.findIndex(((obj: { IP: any; }) => obj.IP == res_ip));
                                        ReportMonitoringBios(true,objIndex,data_rows,res_kdcab,res_kdtk,res_nama,res_ip,res_station,res_request,res_response,IDReport,res_data_code,res_data_msg,result)
                                        rows = data_rows;
                                        setDataProsentase(0)
                                        //-- parse hitung dari data yang terupdate --//
                                        const arr_data_sukses_dan_gagal_from_Table = groupByMessageListeners(rows, 'KODE', false, false, false);
